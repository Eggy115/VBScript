...
Sub GetWwanProfiles(outputFileName)
    On Error Resume Next

    dim wwanProfilesOutputPath, wwanProfilesRootDirectory, wwanProfileDirectories
    wwanProfilesRootDirectory = shell.ExpandEnvironmentStrings("%programdata%\") & "Microsoft\Wwansvc\"
    wwanProfileDirectories = Array("Profiles", "DMProfiles")

   Set outputFile = FSO.OpenTextFile(outputFileName, 2, True)

   for I = 0 to UBound(wwanProfileDirectories)
        wwanProfilesOutputPath = wwanProfilesRootDirectory & wwanProfileDirectories(I)

        exists = fso.FolderExists(wwanProfilesOutputPath)
        if (exists) then
            Set objFolder = fso.GetFolder(wwanProfilesOutputPath)
            objFolder.Delete True
        end if
        ...
        dim titleString
        titleString = "-------------------  + WWAN " & wwanProfileDirectories(I)
        titleString = titleString & " on the System +----------------------"

        outputFile.WriteLine("--------------------------------------------------------------------------")
        outputFile.WriteLine(titleString)
        outputFile.WriteLine("--------------------------------------------------------------------------")
        outputFile.WriteLine("")
        outputFile.WriteLine("")
        ...
        For Each objFile in colFiles
            ...
            outputFile.WriteLine("--------------------------------------------------------------------------")
            Set profileContent = objFile.openasTextStream(1, -2)
            outputFile.WriteLine(profileContent.ReadAll)
...
wwanProfilesLogFileName = configFolder + "\WWANProfiles.txt"
...
Call GetWwanProfiles(wwanProfilesLogFileName)
